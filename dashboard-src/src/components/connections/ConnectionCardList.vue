    ...
    <template #default="{ item }: { item: ConnectionCardListItem }">
      ...
      >
        <div class="min-w-0 flex-1">
          <div class="text-base-content/50 text-xs">{{ t(item.groupKey) }}</div>
          <div
...
type ConnectionCardGroupItem = {
  type: 'group'
  id: string
  groupKey: ConnectionCardGroupKey
  value: string
  count: number
  expanded: boolean
}
...
const groups = computed<ConnectionGroup[]>(() => {
  ...
  for (const connection of renderConnections.value) {
    ...
    else byValue.set(id, { id, groupKey, value, connections: [connection] })
  }
...
const size = computed(() => connectionCardLines.value.length * 28 + 12)
</script>
