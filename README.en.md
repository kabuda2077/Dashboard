...
- `resources/dashboard/`: built frontend assets, generated by `tools/build-zashboard.ps1` and not tracked in git. A fresh clone must build the frontend before the .NET build, otherwise the output has no UI. `build.ps1` and `check.ps1` already run in that order.
